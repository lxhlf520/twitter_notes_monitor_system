...
class Tweet:
    ...
    display_text_range : :class:`list`
        The display text range of the tweet.
    user_mentions : :class:`list`
        The user mentions in the tweet.
    extended_entities : :class:`dict`
        The extended entities of the tweet.
    grok_analysis_button : :class:`bool`
        Whether the Grok analysis button is enabled.
    quick_promote_eligibility : :class:`dict`
        The quick promote eligibility of the tweet.
    unmention_data : :class:`dict`
        The unmention data of the tweet.
    """

    def __init__(self, client: Client = None, data: dict = None, user: User = None) -> None:
        self._client = client
        self._data = data
        self._legacy: dict = self._data['legacy']
        self.user = user or User(client, self._data['core']['user_results']['result'])
    ...
    def created_at(self) -> str:
        # 返回推文的创建时间
        created_at_str = self._legacy['created_at']
        if created_at_str:
            try:
    ...
    @property
    def display_text_range(self) -> list:
        """文本显示范围"""
        return self._legacy.get('display_text_range', [])

    @property
    def user_mentions(self) -> list:
        """提及的用户列表"""
        return self._legacy.get('entities', {}).get('user_mentions', [])

    @property
    def extended_entities(self) -> dict:
        """扩展实体信息"""
        return self._legacy.get('extended_entities', {})

    @property
    def grok_analysis_button(self) -> bool:
        """是否显示Grok分析按钮"""
        return self._data.get('grok_analysis_button', False)

    @property
    def quick_promote_eligibility(self) -> dict:
        """快速推广资格"""
        return self._data.get('quick_promote_eligibility', {})

    @property
    def unmention_data(self) -> dict:
        """未提及数据"""
        return self._data.get('unmention_data', {})
    @property
    def text(self) -> str:
    ...
    @property
    def in_reply_to_post_id(self) -> str | None:
        # 返回被回复推文的ID，如果不是回复则返回None
        return self._legacy.get('in_reply_to_status_id_str',"")
    
    @property
    def in_reply_to_user_id(self) -> str | None:
        # 获取被回复用户的ID，如果不是回复则返回None
    ...
    def quote_count(self) -> int:
        # 返回引用次数
        return self._legacy.get('quote_count', 0)
    ...
    @property
    def media_model(self) -> list[MEDIA_TYPE]:
    ...
    def media(self) -> List:
        ...
        for entry in media_data:
            if entry.get('type') == 'video':
                ...
                video_duration = video_info.get('duration_millis', 0) / 1000
                m.append({
            ...
            elif entry.get('type') == "animated_gif":
                video_info = entry.get('video_info',{})
                variants = video_info.get('variants',[])
                video_url = variants[-1].get('url')
                m.append({
    ...
    @property
    def quote_tweet(self) -> Tweet | None:
        # 返回被引用的推文对象，如果不是引用推文则返回None
        if self._data.get('quoted_status_result'):
            quoted_tweet = self._data['quoted_status_result']
            return tweet_from_data(self._client, quoted_tweet)

    @property
    def ret_tweet(self) -> Tweet | None:
        # 返回被转发的原始推文对象，如果不是转发则返回None
        if self._legacy.get('retweeted_status_result'):
            retweeted_tweet = self._legacy['retweeted_status_result']
            return tweet_from_data(self._client, retweeted_tweet)
    @property
    def source_data(self) -> Dict:
        self._data['tweet_id'] = self.id
        self._data['pub_time'] = self.created_at
        return self._data

    # @property
    # def tweet_info(self) -> Dict:
    #     """
    #     获取推文的基本信息字典
    ...
    #             "user_id": self.user.id,
    #             "post_id": self.id,
    #             "pub_time": self.created_at,
    #             "content": self.full_text,
    #             "url": f"https://x.com/{self.user.screen_name}/status/{self.id}",
    #             "like_count": self.favorite_count,
    #             "quote_count": self.quote_count,
    #             "comment_count": self.reply_count,
    #             "share_count": self.retweet_count,
    #             "view_count": self.view_count,
    #             "lang": self.lang,
    #             "is_retweet_status": int(self.retweeted),
    #             "is_quote_status": int(self.is_quote_status),
    #             "is_reply_status": 1 if self.in_reply_to_post_id else 0,
    #         }
...
def tweet_from_data(client: Client, data: dict) -> Tweet:
    ...
    if 'core' not in tweet_data:
        return None
    # 防护已删除账号、隐私账号或 Twitter 数据异常导致 user_results 缺失
