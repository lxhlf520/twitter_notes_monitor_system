...
class Flow:
    ...
    def execute_task(self, *subtask_inputs, **kwargs) -> None:
        response, _ = self._client.v11.onboarding_task(
            self.guest_token, self.token, list(subtask_inputs), **kwargs
        )
        self.response = response

    def sso_init(self, provider: str) -> None:
        self._client.v11.sso_init(provider, self.guest_token)
...
def cookie_str_to_dict(cookies_str: str) -> dict:
    """将cookie字符串转换为字典"""
    if not cookies_str:
        raise Exception("No cookies available")
    cookie_dict = {}
